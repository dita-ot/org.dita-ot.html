...
>
  ...
  <xsl:template match="node()" mode="jekyll-front-matter">
    ...
    <xsl:if
      test="(/* | /*/*[contains(@class, ' topic/title ')])[tokenize(@outputclass, '\s+') = 'generated']"
    >
  ...
  >
    ...
    <xsl:attribute
      name="id"
      select="replace(lower-case(normalize-space()), ' ', '-')"
    />
    <xsl:next-match>
      <xsl:with-param
        name="default-output-class"
        select="$default-output-class"
      />
    </xsl:next-match>
  </xsl:template>
